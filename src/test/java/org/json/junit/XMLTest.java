...
public class XMLTest {
    ...
    public void testM3WithEmptyString(){
        ...
        try {
            ...
            String expected = "Invalid: transform key to empty string";
            assertEquals(e.getMessage(), expected);
        }
    }

    @Test
    public void testM3WithNull(){
        String xmlStr ="<?xml version=\"1.0\"?>\n" +
                "<catalog>\n" +
                "    <book id=\"bk101\">\n" +
                "        <author>Gambardella, Matthew</author>\n" +
        ...
        try {
            Reader reader = new StringReader(xmlStr);
            JSONObject jo = XML.toJSONObject(reader, s -> null);
            fail("Should have thrown null exception. ");

        }
        catch (JSONException e){
            String expected = "Invalid: transform key to null";
            assertEquals(e.getMessage(), expected);
        }
    }

    @Test
    public void testM3WithDuplicateKeyName(){
        String xmlStr ="<?xml version=\"1.0\"?>\n" +
                "<catalog>\n" +
                "    <book id=\"bk101\">\n" +
                "        <author>Gambardella, Matthew</author>\n" +
        ...
        try {
            Reader reader = new StringReader(xmlStr);
            JSONObject jo = XML.toJSONObject(reader, s -> "random string");
            fail("Should have thrown duplicate exception. ");

        }
        catch (JSONException e){
            String expected = "Invalid: transform key to all the same string";
            assertEquals(e.getMessage(), expected);
        }
    }
}
